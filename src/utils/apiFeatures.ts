...
function mutateFilters(obj: any) {
  Object.keys(obj).forEach(function (key) {
    if (obj[key] !== null && !Array.isArray(obj[key]) && typeof obj[key] === 'object') mutateFilters(obj[key]);
    else if (key === 'match') {
      ...
      if (moment(obj[key], 'YYYY-MM-DD', true).isValid())
        obj[key] = moment.tz(new Date(obj[key]), config.defaultTimeZone).endOf('day').toDate();
    }
  });
}
...
export default class APIFeatures<T extends Document> {
  constructor(
    public query: Query<EnforceDocument<T, {}>[], EnforceDocument<T, {}>, {}, T>,
    private queryString: StringMap,
  ) {
    this.query = query;
